...
LIVE_RESULTS_MESSAGE = md"""ℹ️ Live Results are disabled to safe performance. Checkbox `Plot Results`."""
...
begin 
	...
	ben_rwd = (_x,) -> ReverseDiff.jacobian(x -> model(x), _x);
	jac_rwd = ben_rwd(x1) 
...
begin
	...
	FMIFlux.prepareSolveFMU(fmu, nothing, fmu.type)
	ben_fwd = (_x,) -> ForwardDiff.jacobian(x -> model(x), _x);
	jac_fwd = ben_fwd(x1) 
...
begin
	#getCurrentComponent(fmu).t = tStart
	@btime model(x1)
end
...
# ╔═╡ 82fb4c52-a37a-484c-a7c3-e77d517a9a55
md"""
The resulting Jacobian $A = \frac{\partial \dot{x}}{\partial x}$ is the same of course: 
"""
...
md"""
The same applies for the Jacobian $C = \frac{\partial y}{\partial x}$: 
"""
...
let
	...
	plot(ts[200:end], nfmu_ax[200:end]) #  .- fmu_ax, nfmu_vx)
	plot!(ts[200:end], fmu_ax[200:end])
	plot(ts[200:end], nfmu_ax[200:end] .- fmu_ax[200:end])
	#plot(nfmu_vx[200:end], nfmu_ax[200:end])
	scatter(nfmu_vx[200:end], nfmu_ax[200:end] .- fmu_ax[200:end])
end
...
git-tree-sha1 = "57334feabe2ca2884da73a2b718928dc1d54c791"
